...
write_translated_cc(Ctx, CcFile, Ts) :-
    ...
    writeln(OStream, '  }'),
    writeln(OStream, '  VM::ptr vm = std::make_shared<VM>();'),
    ...
    writeln(OStream, '  Scheduler::getInstance().push_vm(vm);'),
    writeln(OStream, '  for (;;) {'),
    writeln(OStream, '    vm = Scheduler::getInstance().pop_vm();'),
    writeln(OStream, '    if (!vm) {'),
    writeln(OStream, '      break;'),
    writeln(OStream, '    }'),
    writeln(OStream, '    try {'),
    writeln(OStream, '      module(vm, &prog);'),
      ...
      -> Label = label(F / N), ( get(Ctx, Label, L) ; put(Ctx, Label, L) ),
         ( Codes1 = [ Label1 | Codes2], Label1 =.. [label|_]
           -> ( get(Ctx, Label1, L1) ; put(Ctx, Label1, L1) ),
            L1 is Lno + 1,
            Lno1 = L1,
            Ts = [line(Lno, spawn(L, L1, F / N)),
                  label(Lno1, return_from_spawn(F / N)) | Ts2]
         ; Codes2 = Codes1,
           Lno1 is Lno + 1,
           Ts = [line(Lno, spawn(L, Lno1, F / N)),
                 label(Lno1, return_from_spawn(F / N)) | Ts2])
    ),
