...
Q atom(127);  // sift / 2
Q atom(128);  // filter / 3
Q atom(129);  // printstream / 1
Q atom(130);  // printstream / 2
Q atom(131);  // printstream_rest / 2
Q atom(132);  // main / 1
Q atom(133);  // usage:  / 0
Q atom(134);  //  <max> / 0
      ...
      MACRO_requires(1);
{
  ...
  MACRO_proceed;
    case 268:  // primes/2
      ...
      MACRO_spawn(270,269);  // spawn(gen/3)
    case 269:  // return_from_spawn(gen/3)
      MACRO_tail(4);
      MACRO_out_value(reg::x(3),reg::out(1));
      MACRO_out_value(reg::in(2),reg::out(2));
      MACRO_execute(275,2);  // execute(sift/2)
    case 270:  // gen/3
      ...
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_value(reg::in(2),reg::out(2));
      MACRO_call(160,271);  // call((=<)/2)
    case 271:  // return_from_call((=<)/2)
      MACRO_activate;
      MACRO_par(5);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_variable(reg::x(4),reg::out(2));
      MACRO_call(218,272);  // call(__inc__/2)
    case 272:  // return_from_call(__inc__/2)
      ...
      MACRO_out_value(reg::in(2),reg::out(2));
      MACRO_out_value(reg::x(5),reg::out(3));
      MACRO_execute(270,3);  // execute(gen/3)
    case 273:  // gen/3-2
      MACRO_try_guard_else_suspend;
      MACRO_seq(6);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_value(reg::in(2),reg::out(2));
      MACRO_call(161,274);  // call((>)/2)
    ...
    case 275:  // sift/2
      ...
      MACRO_out_value(reg::x(4),reg::out(2));
      MACRO_out_variable(reg::x(3),reg::out(3));
      MACRO_spawn(278,276);  // spawn(filter/3)
    case 276:  // return_from_spawn(filter/3)
      MACRO_tail(6);
      MACRO_out_value(reg::x(3),reg::out(1));
      MACRO_out_value(reg::x(5),reg::out(2));
      MACRO_execute(275,2);  // execute(sift/2)
    ...
    case 279:  // return_from_call((:=)/2)
      ...
      MACRO_call(166,280);  // call((=\=)/2)
    case 280:  // return_from_call((=\=)/2)
      MACRO_activate;
      MACRO_get_list(reg::in(3));
      MACRO_unify_value(reg::x(5));
      MACRO_unify_variable(reg::x(5));
      ...
      MACRO_out_value(reg::x(4),reg::out(2));
      MACRO_out_value(reg::x(5),reg::out(3));
      MACRO_execute(278,3);  // execute(filter/3)
    case 281:  // filter/3-2
    ...
    case 285:  // printstream/1
      ...
      MACRO_par(3);
      MACRO_out_variable(reg::x(2),reg::out(1));
      MACRO_spawn(50,286);  // spawn(outstream/1)
    case 286:  // return_from_spawn(outstream/1)
      MACRO_tail(3);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_value(reg::x(2),reg::out(2));
      MACRO_execute(287,2);  // execute(printstream/2)
    case 287:  // printstream/2
    ...
    case 288:  // printstream/2-2
      ...
      MACRO_out_value(reg::x(3),reg::out(1));
      MACRO_out_value(reg::x(4),reg::out(2));
      MACRO_execute(289,2);  // execute(printstream_rest/2)
    case 289:  // printstream_rest/2
    ...
    case 291:  // main/1
      ...
      MACRO_spawn(44,292);  // spawn(atom_number/2)
    case 292:  // return_from_spawn(atom_number/2)
      MACRO_par(3);
      MACRO_out_structure(atom(125),reg::out(1));
      MACRO_write_value(reg::x(2));
      MACRO_write_variable(reg::x(2));
      MACRO_spawn(6,293);  // spawn(time/1)
    case 293:  // return_from_spawn(time/1)
      MACRO_tail(3);
      MACRO_out_value(reg::x(2),reg::out(1));
      MACRO_execute(285,1);  // execute(printstream/1)
    case 294:  // main/1-2
      MACRO_otherwise;
      MACRO_check_list(reg::in(1));
      MACRO_read_variable(reg::x(2));
...
void setup(Program* prog) {
  ...
  atom(125) = to_atom("primes", 2);
  atom(126) = to_atom("gen", 3);
  atom(127) = to_atom("sift", 2);
  atom(128) = to_atom("filter", 3);
  atom(129) = to_atom("printstream", 1);
  atom(130) = to_atom("printstream", 2);
  atom(131) = to_atom("printstream_rest", 2);
  atom(132) = to_atom("main", 1);
  atom(133) = to_atom("usage: ", 0);
  atom(134) = to_atom(" <max>", 0);
  // setup entry points
  ...
  prog->add_entry_point(atom(125), 268);  // primes / 2
  prog->add_entry_point(atom(126), 270);  // gen / 3
  prog->add_entry_point(atom(127), 275);  // sift / 2
  prog->add_entry_point(atom(128), 278);  // filter / 3
  prog->add_entry_point(atom(129), 285);  // printstream / 1
  prog->add_entry_point(atom(130), 287);  // printstream / 2
  prog->add_entry_point(atom(131), 289);  // printstream_rest / 2
  prog->add_entry_point(atom(132), 291);  // main / 1
}
int
