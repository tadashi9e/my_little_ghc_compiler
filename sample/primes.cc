...
Q atom(123);  // sift / 2
Q atom(124);  // filter / 3
Q atom(125);  // printstream / 1
Q atom(126);  // printstream / 2
Q atom(127);  // printstream_rest / 2
Q atom(128);  // main / 1
Q atom(129);  // usage:  / 0
Q atom(130);  //  <max> / 0
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    atom(121) = to_atom("primes", 2);
    atom(122) = to_atom("gen", 3);
    atom(123) = to_atom("sift", 2);
    atom(124) = to_atom("filter", 3);
    atom(125) = to_atom("printstream", 1);
    atom(126) = to_atom("printstream", 2);
    atom(127) = to_atom("printstream_rest", 2);
    atom(128) = to_atom("main", 1);
    atom(129) = to_atom("usage: ", 0);
    atom(130) = to_atom(" <max>", 0);
    // setup entry points
    ...
    prog->add_entry_point(atom(121), &&L264);  // primes / 2
    prog->add_entry_point(atom(122), &&L266);  // gen / 3
    prog->add_entry_point(atom(123), &&L271);  // sift / 2
    prog->add_entry_point(atom(124), &&L274);  // filter / 3
    prog->add_entry_point(atom(125), &&L281);  // printstream / 1
    prog->add_entry_point(atom(126), &&L283);  // printstream / 2
    prog->add_entry_point(atom(127), &&L285);  // printstream_rest / 2
    prog->add_entry_point(atom(128), &&L287);  // main / 1
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L264:  // primes/2
  ...
  MACRO_spawn(L266, L265);  // spawn(gen/3)
 L265:  // return_from_spawn(gen/3)
  MACRO_tail(4);
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::in(2),reg::out(2));
  MACRO_execute(L271, 2);  // execute(sift/2)
 L266:  // gen/3
  ...
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::in(2),reg::out(2));
  MACRO_call(L156, L267);  // call((=<)/2)
 L267:  // return_from_call((=<)/2)
  MACRO_activate;
  MACRO_par(5);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_variable(reg::x(4),reg::out(2));
  MACRO_call(L214, L268);  // call(__inc__/2)
 L268:  // return_from_call(__inc__/2)
  MACRO_get_list(reg::in(3));
  ...
  MACRO_out_value(reg::in(2),reg::out(2));
  MACRO_out_value(reg::x(5),reg::out(3));
  MACRO_execute(L266, 3);  // execute(gen/3)
 L269:  // gen/3-2
  MACRO_try_guard_else_suspend;
  MACRO_seq(6);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::in(2),reg::out(2));
  MACRO_call(L157, L270);  // call((>)/2)
 L270:  // return_from_call((>)/2)
  MACRO_activate;
  MACRO_get_nil(reg::in(3));
  MACRO_proceed;
 L271:  // sift/2
  MACRO_goal(L271, atom(123));
  MACRO_requires(9);
  MACRO_try_guard_else(L273);
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(3));
  MACRO_read_variable(reg::x(4));
  MACRO_activate;
  ...
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_out_variable(reg::x(3),reg::out(3));
  MACRO_spawn(L274, L272);  // spawn(filter/3)
 L272:  // return_from_spawn(filter/3)
  MACRO_tail(6);
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(5),reg::out(2));
  MACRO_execute(L271, 2);  // execute(sift/2)
 ...
 L274:  // filter/3
  ...
  MACRO_call(L166, L275);  // call((:=)/2)
 L275:  // return_from_call((:=)/2)
  MACRO_seq(7);
  ...
  MACRO_call(L162, L276);  // call((=\=)/2)
 L276:  // return_from_call((=\=)/2)
  MACRO_activate;
  MACRO_get_list(reg::in(3));
  MACRO_unify_value(reg::x(5));
  MACRO_unify_variable(reg::x(5));
  ...
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_out_value(reg::x(5),reg::out(3));
  MACRO_execute(L274, 3);  // execute(filter/3)
 L277:  // filter/3-2
  ...
  MACRO_out_variable(reg::x(6),reg::out(1));
  MACRO_out_constant(tagvalue<TAG_INT>(0),reg::out(2));
  MACRO_call(L166, L278);  // call((:=)/2)
 L278:  // return_from_call((:=)/2)
  MACRO_seq(7);
  MACRO_out_structure(atom(88),reg::out(1));
  MACRO_write_value(reg::x(5));
  MACRO_write_value(reg::in(1));
  MACRO_out_value(reg::x(6),reg::out(2));
  MACRO_call(L159, L279);  // call((=:=)/2)
 L279:  // return_from_call((=:=)/2)
  MACRO_activate;
  MACRO_tail(7);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
 ...
 L280:  // filter/3-3
  ...
  MACRO_proceed;
 L281:  // printstream/1
  MACRO_goal(L281, atom(125));
  MACRO_requires(5);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_par(3);
  MACRO_out_variable(reg::x(2),reg::out(1));
  MACRO_spawn(L46, L282);  // spawn(outstream/1)
 L282:  // return_from_spawn(outstream/1)
  MACRO_tail(3);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::x(2),reg::out(2));
  MACRO_execute(L283, 2);  // execute(printstream/2)
 L283:  // printstream/2
  MACRO_goal(L283, atom(126));
  MACRO_requires(7);
 ...
 L284:  // printstream/2-2
  ...
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_execute(L285, 2);  // execute(printstream_rest/2)
 L285:  // printstream_rest/2
  MACRO_goal(L285, atom(127));
  MACRO_requires(7);
 ...
 L287:  // main/1
  ...
  MACRO_spawn(L40, L288);  // spawn(atom_number/2)
 L288:  // return_from_spawn(atom_number/2)
  MACRO_par(3);
  MACRO_out_structure(atom(121),reg::out(1));
  MACRO_write_value(reg::x(2));
  MACRO_write_variable(reg::x(2));
  MACRO_spawn(L4, L289);  // spawn(time/1)
 L289:  // return_from_spawn(time/1)
  MACRO_tail(3);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_execute(L281, 1);  // execute(printstream/1)
 L290:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
