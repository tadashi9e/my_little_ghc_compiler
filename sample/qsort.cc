...
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    prog->add_entry_point(atom(123), &&L266);  // qsort / 2
    prog->add_entry_point(atom(124), &&L267);  // qsort / 3
    prog->add_entry_point(atom(125), &&L271);  // part / 4
    prog->add_entry_point(atom(126), &&L276);  // atom_number_list / 2
    prog->add_entry_point(atom(127), &&L279);  // printstream / 1
    prog->add_entry_point(atom(128), &&L281);  // printstream / 2
    prog->add_entry_point(atom(129), &&L283);  // printstream_rest / 2
    prog->add_entry_point(atom(130), &&L285);  // main / 1
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L266:  // qsort/2
  ...
  MACRO_out_value(reg::in(2),reg::out(2));
  MACRO_out_nil(reg::out(3));
  MACRO_execute(L267, 3);  // execute(qsort/3)
 L267:  // qsort/3
  MACRO_goal(L267, atom(124));
  MACRO_requires(11);
  ...
  MACRO_spawn(L271, L268);  // spawn(part/4)
 L268:  // return_from_spawn(part/4)
  MACRO_par(7);
  MACRO_out_value(reg::x(6),reg::out(1));
  MACRO_out_value(reg::in(2),reg::out(2));
  ...
  MACRO_write_value(reg::x(4));
  MACRO_write_variable(reg::x(4));
  MACRO_spawn(L267, L269);  // spawn(qsort/3)
 L269:  // return_from_spawn(qsort/3)
  MACRO_tail(7);
  MACRO_out_value(reg::x(5),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_out_value(reg::in(3),reg::out(3));
 ...
 L272:  // return_from_call((<)/2)
  ...
  MACRO_out_value(reg::in(3),reg::out(3));
  MACRO_out_value(reg::x(6),reg::out(4));
  MACRO_execute(L271, 4);  // execute(part/4)
 L273:  // part/4-2
  ...
  MACRO_out_value(reg::in(2),reg::out(1));
  MACRO_out_value(reg::x(6),reg::out(2));
  MACRO_call(L157, L274);  // call((>=)/2)
 L274:  // return_from_call((>=)/2)
  MACRO_activate;
  MACRO_get_list(reg::in(3));
  MACRO_unify_value(reg::x(6));
  MACRO_unify_variable(reg::x(6));
 ...
 L278:  // return_from_spawn(atom_number/2)
  ...
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_execute(L276, 2);  // execute(atom_number_list/2)
 L279:  // printstream/1
  MACRO_goal(L279, atom(127));
  MACRO_requires(5);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_par(3);
  MACRO_out_variable(reg::x(2),reg::out(1));
  MACRO_spawn(L48, L280);  // spawn(outstream/1)
 L280:  // return_from_spawn(outstream/1)
  MACRO_tail(3);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::x(2),reg::out(2));
  MACRO_execute(L281, 2);  // execute(printstream/2)
 L281:  // printstream/2
  MACRO_goal(L281, atom(128));
  MACRO_requires(7);
 ...
 L282:  // printstream/2-2
  ...
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_execute(L283, 2);  // execute(printstream_rest/2)
 L283:  // printstream_rest/2
  MACRO_goal(L283, atom(129));
  MACRO_requires(7);
 ...
 L285:  // main/1
  ...
  MACRO_spawn(L276, L286);  // spawn(atom_number_list/2)
 L286:  // return_from_spawn(atom_number_list/2)
  MACRO_par(4);
  MACRO_out_variable(reg::x(3),reg::out(1));
  MACRO_spawn(L279, L287);  // spawn(printstream/1)
 L287:  // return_from_spawn(printstream/1)
  MACRO_tail(4);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_out_value(reg::x(3),reg::out(2));
  MACRO_execute(L266, 2);  // execute(qsort/2)
 L288:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
