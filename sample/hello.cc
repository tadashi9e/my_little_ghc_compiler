      ...
      MACRO_requires(1);
{
  ...
  MACRO_proceed;
    case 269:  // main/1
      ...
      MACRO_write_variable(reg::x(3));
      MACRO_spawn(54,270,atom(29));  // spawn(outstream/1)
    case 270:  // return_from_spawn(outstream/1)
      MACRO_par(1,4);
      MACRO_out_constant(atom(128),reg::out(1));
      MACRO_spawn(263,271,atom(122));  // spawn(dump_to_dot/1)
    case 271:  // return_from_spawn(dump_to_dot/1)
      MACRO_tail(2,4);
      MACRO_out_value(reg::x(2),reg::out(1));
      MACRO_out_value(reg::x(3),reg::out(2));
      MACRO_execute(272,2);  // execute(writes/2)
    case 272:  // writes/2
      MACRO_goal(272,atom(129));
      MACRO_requires(7);
...
void setup(Program* prog) {
  ...
  prog->add_entry_point(atom(129), 272);  // writes / 2
}
int
