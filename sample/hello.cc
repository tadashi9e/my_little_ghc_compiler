      ...
      MACRO_requires(1);
{
  ...
  MACRO_proceed;
    case 268:  // main/1
      ...
      MACRO_write_variable(reg::x(3));
      MACRO_spawn(50,269);  // spawn(outstream/1)
    case 269:  // return_from_spawn(outstream/1)
      MACRO_par(4);
      MACRO_out_constant(atom(127),reg::out(1));
      MACRO_spawn(262,270);  // spawn(dump_to_dot/1)
    case 270:  // return_from_spawn(dump_to_dot/1)
      MACRO_tail(4);
      MACRO_out_value(reg::x(2),reg::out(1));
      MACRO_out_value(reg::x(3),reg::out(2));
      MACRO_execute(271,2);  // execute(writes/2)
    case 271:  // writes/2
...
void setup(Program* prog) {
  ...
  atom(126) = to_atom("Hello ", 0);
  atom(127) = to_atom("dump/hello.dot", 0);
  atom(128) = to_atom("writes", 2);
  atom(129) = to_atom(" ", 0);
  // setup entry points
  ...
  prog->add_entry_point(atom(128), 271);  // writes / 2
}
int
