...
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    prog->add_entry_point(atom(123), &&L266);  // fibonacci / 2
    prog->add_entry_point(atom(124), &&L267);  // fib / 4
    prog->add_entry_point(atom(125), &&L272);  // printstream / 1
    prog->add_entry_point(atom(126), &&L274);  // printstream / 2
    prog->add_entry_point(atom(127), &&L276);  // printstream_rest / 2
    prog->add_entry_point(atom(128), &&L278);  // main / 1
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L266:  // fibonacci/2
  ...
  MACRO_execute(L267, 4);  // execute(fib/4)
 L267:  // fib/4
  ...
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_value(reg::in(1),reg::out(2));
  MACRO_call(L158, L268);  // call((=<)/2)
 L268:  // return_from_call((=<)/2)
  MACRO_activate;
  ...
  MACRO_call(L176, L269);  // call(__add__/3)
 L269:  // return_from_call(__add__/3)
  MACRO_tail(7);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::in(3),reg::out(2));
 ...
 L270:  // fib/4-2
  ...
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_value(reg::in(1),reg::out(2));
  MACRO_call(L159, L271);  // call((>)/2)
 L271:  // return_from_call((>)/2)
  MACRO_activate;
  MACRO_get_nil(reg::in(4));
  MACRO_proceed;
 L272:  // printstream/1
  MACRO_goal(L272, atom(125));
  MACRO_requires(5);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_par(3);
  MACRO_out_variable(reg::x(2),reg::out(1));
  MACRO_spawn(L48, L273);  // spawn(outstream/1)
 L273:  // return_from_spawn(outstream/1)
  MACRO_tail(3);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::x(2),reg::out(2));
  MACRO_execute(L274, 2);  // execute(printstream/2)
 L274:  // printstream/2
 ...
 L275:  // printstream/2-2
  ...
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_execute(L276, 2);  // execute(printstream_rest/2)
 L276:  // printstream_rest/2
  MACRO_goal(L276, atom(127));
  MACRO_requires(7);
 ...
 L277:  // printstream_rest/2-2
  ...
  MACRO_execute(L276, 2);  // execute(printstream_rest/2)
 L278:  // main/1
  MACRO_goal(L278, atom(128));
  MACRO_requires(6);
  MACRO_try_guard_else(L281);
  MACRO_check_list(reg::in(1));
  MACRO_read_void;
  MACRO_read_list;
  MACRO_read_variable(reg::x(2));
 ...
 L279:  // return_from_spawn(atom_number/2)
  MACRO_par(4);
  MACRO_out_variable(reg::x(3),reg::out(1));
  MACRO_spawn(L272, L280);  // spawn(printstream/1)
 L280:  // return_from_spawn(printstream/1)
  MACRO_tail(4);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_out_value(reg::x(3),reg::out(2));
  MACRO_execute(L266, 2);  // execute(fibonacci/2)
 L281:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
