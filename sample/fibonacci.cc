...
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    prog->add_entry_point(atom(121), &&L264);  // fibonacci / 2
    prog->add_entry_point(atom(122), &&L265);  // fib / 4
    prog->add_entry_point(atom(123), &&L270);  // printstream / 1
    prog->add_entry_point(atom(124), &&L272);  // printstream / 2
    prog->add_entry_point(atom(125), &&L274);  // printstream_rest / 2
    prog->add_entry_point(atom(126), &&L276);  // main / 1
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L264:  // fibonacci/2
  ...
  MACRO_execute(L265, 4);  // execute(fib/4)
 L265:  // fib/4
  ...
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_value(reg::in(1),reg::out(2));
  MACRO_call(L156, L266);  // call((=<)/2)
 L266:  // return_from_call((=<)/2)
  MACRO_activate;
  ...
  MACRO_call(L174, L267);  // call(__add__/3)
 L267:  // return_from_call(__add__/3)
  MACRO_tail(7);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::in(3),reg::out(2));
 ...
 L268:  // fib/4-2
  ...
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_value(reg::in(1),reg::out(2));
  MACRO_call(L157, L269);  // call((>)/2)
 L269:  // return_from_call((>)/2)
  MACRO_activate;
  MACRO_get_nil(reg::in(4));
  MACRO_proceed;
 L270:  // printstream/1
  MACRO_goal(L270, atom(123));
  MACRO_requires(5);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_par(3);
  MACRO_out_variable(reg::x(2),reg::out(1));
  MACRO_spawn(L46, L271);  // spawn(outstream/1)
 L271:  // return_from_spawn(outstream/1)
  MACRO_tail(3);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_value(reg::x(2),reg::out(2));
  MACRO_execute(L272, 2);  // execute(printstream/2)
 L272:  // printstream/2
  MACRO_goal(L272, atom(124));
  MACRO_requires(7);
 ...
 L273:  // printstream/2-2
  ...
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_out_value(reg::x(4),reg::out(2));
  MACRO_execute(L274, 2);  // execute(printstream_rest/2)
 L274:  // printstream_rest/2
  MACRO_goal(L274, atom(125));
  MACRO_requires(7);
 ...
 L276:  // main/1
  ...
  MACRO_spawn(L40, L277);  // spawn(atom_number/2)
 L277:  // return_from_spawn(atom_number/2)
  MACRO_par(4);
  MACRO_out_variable(reg::x(3),reg::out(1));
  MACRO_spawn(L270, L278);  // spawn(printstream/1)
 L278:  // return_from_spawn(printstream/1)
  MACRO_tail(4);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_out_value(reg::x(3),reg::out(2));
  MACRO_execute(L264, 2);  // execute(fibonacci/2)
 L279:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
