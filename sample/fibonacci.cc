...
Q atom(126);  // fibonacci / 2
Q atom(127);  // fib / 4
Q atom(128);  // printstream / 1
Q atom(129);  // printstream / 2
Q atom(130);  // printstream_rest / 2
Q atom(131);  // main / 1
Q atom(132);  // usage:  / 0
Q atom(133);  //  <Max> / 0
      ...
      MACRO_requires(1);
{
  ...
  MACRO_proceed;
    case 273:  // fibonacci/2
      ...
      MACRO_execute(274,4);  // execute(fib/4)
    case 274:  // fib/4
      ...
      MACRO_out_value(reg::in(3),reg::out(1));
      MACRO_out_value(reg::in(1),reg::out(2));
      MACRO_call(165,275,atom(84));  // call((=<)/2)
    case 275:  // return_from_call((=<)/2)
      ...
      MACRO_out_value(reg::in(3),reg::out(2));
      MACRO_out_variable(reg::y(5),reg::out(3));
      MACRO_spawn(183,276,atom(45));  // spawn(__add__/3)
    case 276:  // return_from_spawn(__add__/3)
      MACRO_tail(4,7);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_value(reg::in(3),reg::out(2));
      MACRO_out_value(reg::y(5),reg::out(3));
    ...
    case 277:  // fib/4-2
      ...
      MACRO_out_value(reg::in(3),reg::out(1));
      MACRO_out_value(reg::in(1),reg::out(2));
      MACRO_call(166,278,atom(85));  // call((>)/2)
    case 278:  // return_from_call((>)/2)
      MACRO_activate;
      MACRO_get_nil(reg::in(4));
      MACRO_proceed;
    case 279:  // printstream/1
      MACRO_goal(279,atom(128));
      MACRO_requires(5);
      MACRO_try_guard_else_suspend;
      MACRO_activate;
      MACRO_par(1,3);
      MACRO_out_variable(reg::y(2),reg::out(1));
      MACRO_spawn(54,280,atom(29));  // spawn(outstream/1)
    case 280:  // return_from_spawn(outstream/1)
      MACRO_tail(2,3);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_value(reg::y(2),reg::out(2));
      MACRO_execute(281,2);  // execute(printstream/2)
    case 281:  // printstream/2
      MACRO_goal(281,atom(129));
      MACRO_requires(7);
    ...
    case 286:  // return_from_spawn(atom_number/2)
      MACRO_par(1,4);
      MACRO_out_variable(reg::y(3),reg::out(1));
      MACRO_spawn(279,287,atom(128));  // spawn(printstream/1)
    case 287:  // return_from_spawn(printstream/1)
      MACRO_tail(2,4);
      MACRO_out_value(reg::y(2),reg::out(1));
      MACRO_out_value(reg::y(3),reg::out(2));
      MACRO_execute(273,2);  // execute(fibonacci/2)
    case 288:  // main/1-2
      MACRO_otherwise;
      MACRO_check_list(reg::in(1));
      MACRO_read_variable(reg::x(4));
...
void setup(Program* prog) {
  ...
  atom(126) = to_atom("fibonacci", 2);
  atom(127) = to_atom("fib", 4);
  atom(128) = to_atom("printstream", 1);
  atom(129) = to_atom("printstream", 2);
  atom(130) = to_atom("printstream_rest", 2);
  atom(131) = to_atom("main", 1);
  atom(132) = to_atom("usage: ", 0);
  atom(133) = to_atom(" <Max>", 0);
  // setup entry points
  ...
  prog->add_entry_point(atom(126), 273);  // fibonacci / 2
  prog->add_entry_point(atom(127), 274);  // fib / 4
  prog->add_entry_point(atom(128), 279);  // printstream / 1
  prog->add_entry_point(atom(129), 281);  // printstream / 2
  prog->add_entry_point(atom(130), 283);  // printstream_rest / 2
  prog->add_entry_point(atom(131), 285);  // main / 1
}
int
