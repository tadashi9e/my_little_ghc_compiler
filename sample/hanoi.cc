...
Q atom(122);  // usage:  / 0
Q atom(123);  //  <N> / 0
Q atom(124);  // report_results / 2
Q atom(125);  // report_results / 3
Q atom(126);  // report1 / 3
Q atom(127);  // : / 0
Q atom(128);  // go / 2
Q atom(129);  // left / 0
Q atom(130);  // center / 0
...
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    atom(122) = to_atom("usage: ", 0);
    atom(123) = to_atom(" <N>", 0);
    atom(124) = to_atom("report_results", 2);
    atom(125) = to_atom("report_results", 3);
    atom(126) = to_atom("report1", 3);
    atom(127) = to_atom(":", 0);
    atom(128) = to_atom("go", 2);
    atom(129) = to_atom("left", 0);
    atom(130) = to_atom("center", 0);
    atom(131) = to_atom("right", 0);
    atom(132) = to_atom("move", 6);
    atom(133) = to_atom("m", 2);
    // setup entry points
    ...
    prog->add_entry_point(atom(124), &&L270);  // report_results / 2
    prog->add_entry_point(atom(125), &&L271);  // report_results / 3
    prog->add_entry_point(atom(126), &&L274);  // report1 / 3
    prog->add_entry_point(atom(128), &&L275);  // go / 2
    prog->add_entry_point(atom(132), &&L276);  // move / 6
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L265:  // return_from_spawn(atom_number/2)
  MACRO_par(4);
  MACRO_out_variable(reg::x(3),reg::out(1));
  MACRO_spawn(L46, L266);  // spawn(outstream/1)
 L266:  // return_from_spawn(outstream/1)
  MACRO_par(4);
  MACRO_out_structure(atom(85),reg::out(1));
  MACRO_write_value(reg::x(3));
  MACRO_write_nil;
  MACRO_out_variable(reg::x(3),reg::out(2));
  MACRO_spawn(L270, L267);  // spawn(report_results/2)
 L267:  // return_from_spawn(report_results/2)
  MACRO_par(4);
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_spawn(L7, L268);  // spawn(wait_time/1)
 L268:  // return_from_spawn(wait_time/1)
  MACRO_tail(4);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_out_value(reg::x(3),reg::out(2));
  MACRO_execute(L275, 2);  // execute(go/2)
 L269:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
  ...
  MACRO_write_nil;
  MACRO_execute(L46, 1);  // execute(outstream/1)
 L270:  // report_results/2
  MACRO_goal(L270, atom(124));
  MACRO_requires(8);
  MACRO_try_guard_else_suspend;
  ...
  MACRO_execute(L271, 3);  // execute(report_results/3)
 L271:  // report_results/3
  MACRO_goal(L271, atom(125));
  MACRO_requires(11);
  MACRO_try_guard_else_suspend;
  MACRO_check_structure(atom(85),reg::in(1));
  MACRO_read_variable(reg::x(6));
  ...
  MACRO_out_value(reg::in(3),reg::out(3));
  MACRO_spawn(L274, L272);  // spawn(report1/3)
 L272:  // return_from_spawn(report1/3)
  MACRO_par(8);
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_variable(reg::x(7),reg::out(2));
  MACRO_call(L214, L273);  // call(__inc__/2)
 L273:  // return_from_call(__inc__/2)
  MACRO_tail(8);
  MACRO_out_structure(atom(85),reg::out(1));
  MACRO_write_value(reg::x(6));
  MACRO_write_value(reg::x(4));
 ...
 L274:  // report1/3
  ...
  MACRO_proceed;
 L275:  // go/2
  MACRO_goal(L275, atom(128));
  MACRO_requires(9);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_tail(4);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_constant(atom(129),reg::out(2));
  MACRO_out_constant(atom(130),reg::out(3));
  MACRO_out_constant(atom(131),reg::out(4));
  MACRO_out_value(reg::in(2),reg::out(5));
  MACRO_out_nil(reg::out(6));
  MACRO_execute(L276, 6);  // execute(move/6)
 L276:  // move/6
  MACRO_goal(L276, atom(132));
  MACRO_requires(15);
 ...
 L277:  // move/6-2
  ...
  MACRO_call(L157, L278);  // call((>)/2)
 L278:  // return_from_call((>)/2)
  MACRO_activate;
  MACRO_par(8);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_variable(reg::x(7),reg::out(2));
  MACRO_call(L218, L279);  // call(__dec__/2)
 L279:  // return_from_call(__dec__/2)
  MACRO_par(9);
  ...
  MACRO_write_structure(atom(133));
  MACRO_write_value(reg::in(2));
  MACRO_write_value(reg::in(3));
  MACRO_write_variable(reg::x(8));
  MACRO_spawn(L276, L280);  // spawn(move/6)
 L280:  // return_from_spawn(move/6)
  MACRO_tail(9);
  MACRO_out_value(reg::x(7),reg::out(1));
