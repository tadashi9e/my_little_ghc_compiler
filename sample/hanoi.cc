...
Q atom(126);  // usage:  / 0
Q atom(127);  //  <N> / 0
Q atom(128);  // report_results / 2
Q atom(129);  // report_results / 3
Q atom(130);  // report1 / 3
Q atom(131);  // : / 0
Q atom(132);  // go / 2
Q atom(133);  // left / 0
Q atom(134);  // center / 0
      ...
      MACRO_requires(1);
{
  ...
  MACRO_proceed;
    ...
    case 269:  // return_from_spawn(atom_number/2)
      MACRO_par(4);
      MACRO_out_variable(reg::x(3),reg::out(1));
      MACRO_spawn(50,270);  // spawn(outstream/1)
    case 270:  // return_from_spawn(outstream/1)
      ...
      MACRO_write_nil;
      MACRO_out_variable(reg::x(3),reg::out(2));
      MACRO_spawn(274,271);  // spawn(report_results/2)
    case 271:  // return_from_spawn(report_results/2)
      MACRO_par(4);
      MACRO_out_value(reg::x(3),reg::out(1));
      MACRO_spawn(9,272);  // spawn(wait_time/1)
    case 272:  // return_from_spawn(wait_time/1)
      MACRO_tail(4);
      MACRO_out_value(reg::x(2),reg::out(1));
      MACRO_out_value(reg::x(3),reg::out(2));
      MACRO_execute(279,2);  // execute(go/2)
    case 273:  // main/1-2
      MACRO_otherwise;
      MACRO_check_list(reg::in(1));
      MACRO_read_variable(reg::x(2));
    ...
    case 274:  // report_results/2
      ...
      MACRO_execute(275,3);  // execute(report_results/3)
    case 275:  // report_results/3
      ...
      MACRO_out_value(reg::in(3),reg::out(3));
      MACRO_spawn(278,276);  // spawn(report1/3)
    case 276:  // return_from_spawn(report1/3)
      MACRO_par(8);
      MACRO_out_value(reg::in(3),reg::out(1));
      MACRO_out_variable(reg::x(7),reg::out(2));
      MACRO_call(218,277);  // call(__inc__/2)
    ...
    case 279:  // go/2
      ...
      MACRO_tail(4);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_constant(atom(133),reg::out(2));
      MACRO_out_constant(atom(134),reg::out(3));
      MACRO_out_constant(atom(135),reg::out(4));
      MACRO_out_value(reg::in(2),reg::out(5));
      MACRO_out_nil(reg::out(6));
      MACRO_execute(280,6);  // execute(move/6)
    case 280:  // move/6
      MACRO_goal(280,atom(136));
      MACRO_requires(15);
    ...
    case 281:  // move/6-2
      ...
      MACRO_call(161,282);  // call((>)/2)
    case 282:  // return_from_call((>)/2)
      MACRO_activate;
      MACRO_par(8);
      MACRO_out_value(reg::in(1),reg::out(1));
      MACRO_out_variable(reg::x(7),reg::out(2));
      MACRO_call(222,283);  // call(__dec__/2)
    case 283:  // return_from_call(__dec__/2)
      MACRO_par(9);
      ...
      MACRO_write_structure(atom(137));
      MACRO_write_value(reg::in(2));
      MACRO_write_value(reg::in(3));
      MACRO_write_variable(reg::x(8));
      MACRO_spawn(280,284);  // spawn(move/6)
    case 284:  // return_from_spawn(move/6)
      MACRO_tail(9);
      MACRO_out_value(reg::x(7),reg::out(1));
...
void setup(Program* prog) {
  ...
  atom(126) = to_atom("usage: ", 0);
  atom(127) = to_atom(" <N>", 0);
  atom(128) = to_atom("report_results", 2);
  atom(129) = to_atom("report_results", 3);
  atom(130) = to_atom("report1", 3);
  atom(131) = to_atom(":", 0);
  atom(132) = to_atom("go", 2);
  atom(133) = to_atom("left", 0);
  atom(134) = to_atom("center", 0);
  atom(135) = to_atom("right", 0);
  atom(136) = to_atom("move", 6);
  atom(137) = to_atom("m", 2);
  // setup entry points
  ...
  prog->add_entry_point(atom(128), 274);  // report_results / 2
  prog->add_entry_point(atom(129), 275);  // report_results / 3
  prog->add_entry_point(atom(130), 278);  // report1 / 3
  prog->add_entry_point(atom(132), 279);  // go / 2
  prog->add_entry_point(atom(136), 280);  // move / 6
}
int
