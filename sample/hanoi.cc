...
Q atom(124);  // usage:  / 0
Q atom(125);  //  <N> / 0
Q atom(126);  // report_results / 2
Q atom(127);  // report_results / 3
Q atom(128);  // report1 / 3
Q atom(129);  // : / 0
Q atom(130);  // go / 2
Q atom(131);  // left / 0
Q atom(132);  // center / 0
...
void module(VM::ptr vm, Program* prog, bool setup) {
  if (setup) {
    ...
    atom(124) = to_atom("usage: ", 0);
    atom(125) = to_atom(" <N>", 0);
    atom(126) = to_atom("report_results", 2);
    atom(127) = to_atom("report_results", 3);
    atom(128) = to_atom("report1", 3);
    atom(129) = to_atom(":", 0);
    atom(130) = to_atom("go", 2);
    atom(131) = to_atom("left", 0);
    atom(132) = to_atom("center", 0);
    atom(133) = to_atom("right", 0);
    atom(134) = to_atom("move", 6);
    atom(135) = to_atom("m", 2);
    // setup entry points
    ...
    prog->add_entry_point(atom(126), &&L272);  // report_results / 2
    prog->add_entry_point(atom(127), &&L273);  // report_results / 3
    prog->add_entry_point(atom(128), &&L276);  // report1 / 3
    prog->add_entry_point(atom(130), &&L277);  // go / 2
    prog->add_entry_point(atom(134), &&L278);  // move / 6
    vm->FAIL = &&LFAIL;
    vm->FAIL2 = &&LFAIL2;
 ...
 L267:  // return_from_spawn(atom_number/2)
  MACRO_par(4);
  MACRO_out_variable(reg::x(3),reg::out(1));
  MACRO_spawn(L48, L268);  // spawn(outstream/1)
 L268:  // return_from_spawn(outstream/1)
  MACRO_par(4);
  MACRO_out_structure(atom(87),reg::out(1));
  MACRO_write_value(reg::x(3));
  MACRO_write_nil;
  MACRO_out_variable(reg::x(3),reg::out(2));
  MACRO_spawn(L272, L269);  // spawn(report_results/2)
 L269:  // return_from_spawn(report_results/2)
  MACRO_par(4);
  MACRO_out_value(reg::x(3),reg::out(1));
  MACRO_spawn(L9, L270);  // spawn(wait_time/1)
 L270:  // return_from_spawn(wait_time/1)
  MACRO_tail(4);
  MACRO_out_value(reg::x(2),reg::out(1));
  MACRO_out_value(reg::x(3),reg::out(2));
  MACRO_execute(L277, 2);  // execute(go/2)
 L271:  // main/1-2
  MACRO_otherwise;
  MACRO_check_list(reg::in(1));
  MACRO_read_variable(reg::x(2));
  ...
  MACRO_write_nil;
  MACRO_execute(L48, 1);  // execute(outstream/1)
 L272:  // report_results/2
  MACRO_goal(L272, atom(126));
  MACRO_requires(8);
  MACRO_try_guard_else_suspend;
  ...
  MACRO_execute(L273, 3);  // execute(report_results/3)
 L273:  // report_results/3
  ...
  MACRO_out_value(reg::in(3),reg::out(3));
  MACRO_spawn(L276, L274);  // spawn(report1/3)
 L274:  // return_from_spawn(report1/3)
  MACRO_par(8);
  MACRO_out_value(reg::in(3),reg::out(1));
  MACRO_out_variable(reg::x(7),reg::out(2));
  MACRO_call(L216, L275);  // call(__inc__/2)
 L275:  // return_from_call(__inc__/2)
  MACRO_tail(8);
  MACRO_out_structure(atom(87),reg::out(1));
  MACRO_write_value(reg::x(6));
  MACRO_write_value(reg::x(4));
 ...
 L276:  // report1/3
  ...
  MACRO_proceed;
 L277:  // go/2
  MACRO_goal(L277, atom(130));
  MACRO_requires(9);
  MACRO_try_guard_else_suspend;
  MACRO_activate;
  MACRO_tail(4);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_constant(atom(131),reg::out(2));
  MACRO_out_constant(atom(132),reg::out(3));
  MACRO_out_constant(atom(133),reg::out(4));
  MACRO_out_value(reg::in(2),reg::out(5));
  MACRO_out_nil(reg::out(6));
  MACRO_execute(L278, 6);  // execute(move/6)
 L278:  // move/6
  MACRO_goal(L278, atom(134));
  MACRO_requires(15);
 ...
 L279:  // move/6-2
  ...
  MACRO_call(L159, L280);  // call((>)/2)
 L280:  // return_from_call((>)/2)
  MACRO_activate;
  MACRO_par(8);
  MACRO_out_value(reg::in(1),reg::out(1));
  MACRO_out_variable(reg::x(7),reg::out(2));
  MACRO_call(L220, L281);  // call(__dec__/2)
 L281:  // return_from_call(__dec__/2)
  MACRO_par(9);
  ...
  MACRO_write_structure(atom(135));
  MACRO_write_value(reg::in(2));
  MACRO_write_value(reg::in(3));
  MACRO_write_variable(reg::x(8));
  MACRO_spawn(L278, L282);  // spawn(move/6)
 L282:  // return_from_spawn(move/6)
  MACRO_tail(9);
  MACRO_out_value(reg::x(7),reg::out(1));
